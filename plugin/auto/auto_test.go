...
// TODO: consider variables reuse，move to pkg/test?
var miekAuth = []dns.RR{
...
var dnsTestCases = []test.Case{
	...
	{
		Qname: "b.miek.failure.", Qtype: dns.TypeA,
...
func TestLookup(t *testing.T) {
	zone, err := file.Parse(strings.NewReader(dbMiekNL), testzone, "stdin", 0)
	if err != nil {
		t.Fatalf("Expected no error when reading zone, got %q", err)
	}
	auto := Auto{Next: test.ErrorHandler(), Zones: &Zones{Z: map[string]*file.Zone{testzone: zone}, names: []string{testzone}, origins: []string{testzone}}}
	ctx := context.TODO()
	...
	for _, tc := range dnsTestCases {
		...
		_, err := auto.ServeDNS(ctx, rec, req)
		if err != nil {
